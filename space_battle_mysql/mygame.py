...
mycursor = db.cursor()
Q1 = "CREATE TABLE Account (id int PRIMARY KEY AUTO_INCREMENT, name VARCHAR(50), score int DEFAULT 0)"
            ...
            if event.type == pygame.KEYDOWN:
                if active:
                    if event.key == pygame.K_RETURN:
                        mycursor.execute("SELECT name FROM Account WHERE name = %s", (account,))
                        exist = mycursor.fetchone()
                        if not exist:
                            mycursor.execute("INSERT INTO Account (name) VALUE (%s)", (account,))
                            db.commit()
                            mycursor.execute("SELECT name FROM Account WHERE name = %s", (account,))
                            exist = mycursor.fetchone()
                            user = exist[0] # 紀錄目前玩家
                        ...
                        account += event.unicode
        text_surface = font.render(account, True, WHITE)
        input_box_width = max(200, text_surface.get_width())
        input_box.w = input_box_width
        input_box.center = (WIDTH/2, HEIGHT*2/3)
        mycursor.execute("SELECT score FROM Account WHERE name = %s", (user,))
        show_score = mycursor.fetchone()

        screen.blit(background_img, (0, 0))
        draw_text(screen, "太空生存戰", 64, WHITE, WIDTH/2, HEIGHT/4)
        draw_text(screen, "w a s d移動飛船 空白鍵射子彈", 22, WHITE, WIDTH/2, HEIGHT/2)
        draw_text(screen, "按下ENTER開始遊戲", 18, WHITE, WIDTH/2, HEIGHT*3/4) 
        if show_score != None:
            myscore = show_score[0]
            draw_text(screen, user + f"目前最高分數: {myscore}", 30, WHITE, WIDTH/2, HEIGHT*1/6)
        draw_text(screen, "USER NAME:", 20, WHITE, 90, 385)
        screen.blit(text_surface, (input_box.x+5, input_box.y+3))
        pygame.draw.rect(screen, WHITE, input_box, -1) # 畫方框
...
class Rock(pygame.sprite.Sprite):
    ...
    def update(self):
        self.rotate()
        self.rect.y += self.speedy + level 
        self.rect.x += self.speedx
        if self.rect.top > HEIGHT or self.rect.left > WIDTH or self.rect.right < 0:
            self.rect.x = random.randrange(0, WIDTH - self.rect.width)
            self.rect.y = random.randrange(-100, -40)
            self.speedy = random.randrange(2, 6) + level
            self.speedx = random.randrange(-3, 3)
...
while running:
    ...
    # update
    if init:
        now = pygame.time.get_ticks() - init_time 
        if now > last and now - last > 15000:
            level += 1
            last = now
    else:
        now = pygame.time.get_ticks() 
    ...
    if player.lives == 0 and not(die.alive()): # alive() 判斷是否播完
        mycursor.execute("SELECT name, score FROM Account WHERE (name = %s)", (user,))
        old_value = mycursor.fetchone()
        old_score = old_value[1]
        if score > old_score:
            best_score = score
            mycursor.execute("UPDATE Account SET score = %s WHERE name = %s", (best_score, user))
            db.commit()
        show_init = True
        init = False
    ...
    draw_text(screen, str(int(now/1000)), 18, RED, WIDTH/2+30, 10)
    draw_text(screen, "level: "+str(level), 18, WHITE, WIDTH/2+80, 10)
