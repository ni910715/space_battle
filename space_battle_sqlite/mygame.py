...
Q1 = "CREATE TABLE Account (id INTEGER PRIMARY KEY, name TEXT, score INTEGER DEFAULT 0)"
...
def draw_menu():
    ...
    while waiting:
            ...
            if event.type == pygame.KEYDOWN:
                if active:
                    if event.key == pygame.K_RETURN:
                        mycursor.execute("SELECT name FROM Account WHERE name = ?", (account,))
                        exist = mycursor.fetchone()
                        if not exist:
                            mycursor.execute("INSERT INTO Account (name) VALUES (?)", (account,))
                            db.commit()
                            mycursor.execute("SELECT name FROM Account WHERE name = ?", (account,))
                            exist = mycursor.fetchone()
                            user = exist[0] # 紀錄目前玩家
        ...
        mycursor.execute("SELECT score FROM Account WHERE name = ?", (user,))
...
while running:
    ...
    if player.lives == 0 and not(die.alive()): # alive() 判斷是否播完
        mycursor.execute("SELECT name, score FROM Account WHERE (name = ?)", (user,))
        old_value = mycursor.fetchone()
        old_score = old_value[1]
        if score > old_score:
            best_score = score
            mycursor.execute("UPDATE Account SET score = ? WHERE name = ?", (best_score, user))
            db.commit()
        show_init = True
        init = False
